    ...
    for (let user of $.userList) {
        ...
        try {
            ...
            if (user.ckStatus) {
                ...
                DoubleLog(isMultiUser ? `👤 ${user.userName || `账号${user.index}`}：请在浏览器中重新登录 sb.sb 以更新凭据。` : "请在浏览器中重新登录 sb.sb 以更新凭据。");
            }
...
async function checkEnv() { try { if (!userCookie?.length) throw new Error("未检测到有效账号数据！请先在浏览器中登录 sb.sb 并访问论坛页面捕获 Cookie。"); $.log(`\n[INFO] 检测到 ${userCookie?.length ?? 0} 个账号\n`), $.userList.push(...userCookie.map((o => new UserInfo(o))).filter(Boolean)) } catch (o) { throw o } }
