...
function Watermark({ provider, size }: { provider: ProviderBrand; size: number }) {
  const name = provider === "grok" ? "watermark-grok.png" : provider === "claude" ? "watermark-claude.png" : "watermark-chatgpt.png"
  return <Image filePath={`${Script.directory}/assets/${name}`} resizable scaleToFit renderingMode="template" foregroundStyle={C.watermark} frame={{ width: size, height: size }}/>
}
